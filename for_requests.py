...
from for_date import crate_str, create_next_str
import re
...
def get_timetable_next_week():
    ...
    response_get = requests.get(url)
    with open("next_week.html", "w") as file:
        file.write(response_get.text)
    ...
    tag = soup.find(text=re.compile(f'{create_next_str()}')).parent
    pers = tag.get('value')
    ...
    response = requests.post(url, date)
    # if response.ok:
    with open ("next_week_2.html", "w") as file:
        file.write(response.text)

    with open ("next_week_2.html") as file:
        src = file.read()
    ...
    with open ("next_week_timetable.html", "w") as file:
        file.write(response_time_table.text)